...
var @event = new Event
{
    ...
    {
        DateTime = $"{DateTime.UtcNow.AddMinutes(5).ToString("yyyy-MM-ddThh:mm:ss")}",
        TimeZone = $"{TimeZoneInfo.Local.Id}"
    },
    End = new DateTimeTimeZone
    {
        DateTime = $"{DateTime.UtcNow.AddMinutes(30).ToString("yyyy-MM-ddThh:mm:ss")}",
        TimeZone = $"{TimeZoneInfo.Local.Id}"
    },
...
DelegateAuthenticationProvider authProvider = new DelegateAuthenticationProvider(async (request) =>
{
    try
    {
        ...
        try
        {
            var result = await ObtainTokenAsync(app);
...
var request = graphClient.Me.Events.Request();
var calendarEvent = await request.AddAsync(@event);
