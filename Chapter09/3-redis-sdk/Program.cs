...
namespace TheCloudShopsCache
{
    class Program
    {
        ...
        private static async Task<Client> GeneralCachingApproach(int ClientId)
        {
            ...
            if (await cache.KeyExistsAsync(theKey)) 
            { 
            ...
            {
                Client client = LoadClientFromDB();
        ...
        private static async Task PlayWithSetItem()
        {
            ...
            await cache.SetAddAsync(key, value1).ContinueWith(a => Console.WriteLine($"Add another {value1}")); 
